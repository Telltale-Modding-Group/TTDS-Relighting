...
local OnPress_AgentDuplicate = function(textButton_button)
    local string_selectedAgentName = AgentGetName(agent_currentSelectedAgent);
    local vector_selectedAgentWorldPosition = AgentGetWorldPos(agent_currentSelectedAgent);
    local vector_selectedAgentWorldRotation = AgentGetWorldRot(agent_currentSelectedAgent);

    local bool_duplicateChildren = false;
    local bool_copySceneProps = true;
    local bool_copyClasses = true;
    agent_currentSelectedAgent = AgentDuplicate("DUPLICATE_" .. string_selectedAgentName, agent_currentSelectedAgent, bool_duplicateChildren, bool_copySceneProps, bool_copyClasses);
    AgentSetWorldPos(agent_currentSelectedAgent, vector_selectedAgentWorldPosition);
    AgentSetWorldRot(agent_currentSelectedAgent, vector_selectedAgentWorldRotation);
end
...
local OnPress_AgentRename = function(textButton_button)
    ...
    --local string_newName = TLSE_Development_Editor_GetTextBoxString(AgentGetName(agent_currentSelectedAgent), "Rename Agent");

    --if(string_newName ~= nil) then
        --AgentSetName(agent_currentSelectedAgent, string_newName);
    --end
end

TLSE_Development_MainGUI_TabInspectorInitalize = function()
    ...
    textButton_agentDelete = TLSE_Development_Editor_GUI_CreateTextButton("[DELETE AGENT]", false, Vector(0.685, 0.705, 0.0), OnPress_AgentDelete, nil);
    textButton_agentDuplicate = TLSE_Development_Editor_GUI_CreateTextButton("[DUPLICATE AGENT]", false, Vector(0.685, 0.725, 0.0), OnPress_AgentDuplicate, nil);
    textButton_agentMoveToCamera = TLSE_Development_Editor_GUI_CreateTextButton("[MOVE AGENT TO CAMERA]", false, Vector(0.685, 0.745, 0.0), OnPress_AgentMoveToCamera, nil);
...
TLSE_Development_MainGUI_TabInspectorUpdate = function()
